...
function GradientButton({ children, className = "", ...props }) {
  return <button className={`inspira-gradient-button ${className}`} {...props}><span className="inspira-gradient-button-content">{children}</span></button>;
}

const Card3DContext = React.createContext(false);

function Card3D({ children, className = "", containerClassName = "" }) {
  ...
  const move = (event) => {
    ...
    element.style.transform = `rotateY(${(event.clientX - left - width / 2) / 25}deg) rotateX(${(event.clientY - top - height / 2) / 25}deg)`;
  };
  const leave = () => {
    setEntered(false);
    if (ref.current) ref.current.style.transform = "rotateY(0deg) rotateX(0deg)";
  };
  return <Card3DContext.Provider value={entered}><div className={`inspira-card-container ${containerClassName}`}><div ref={ref} className={`inspira-card-body ${className}`} onMouseEnter={() => setEntered(true)} onMouseMove={move} onMouseLeave={leave}>{children}</div></div></Card3DContext.Provider>;
}

function Card3DItem({ children, className = "", translateX = 0, translateY = 0, translateZ = 0, rotateX = 0, rotateY = 0, rotateZ = 0 }) {
  const entered = React.useContext(Card3DContext);
  const transform = entered ? `translateX(${translateX}px) translateY(${translateY}px) translateZ(${translateZ}px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) rotateZ(${rotateZ}deg)` : "translateX(0px) translateY(0px) translateZ(0px) rotateX(0deg) rotateY(0deg) rotateZ(0deg)";
  return <div className={`inspira-card-item ${className}`} style={{ transform }}>{children}</div>;
}

function useInspiraPanelTilt() {
  useEffect(() => {
    const panels = Array.from(document.querySelectorAll(".panel, .hero-strip, .page-title-row, .drift-banner, .arena-command-hero, .arena-summary-strip, .arena-command-surface, .arena-telemetry-surface, .arena-signal-surface"));
    const handlers = panels.map((panel) => {
      panel.classList.add("inspira-panel-surface");
      const onMove = (event) => {
        const { left, top, width, height } = panel.getBoundingClientRect();
        const rotateY = (event.clientX - left - width / 2) / 48;
        const rotateX = (event.clientY - top - height / 2) / -48;
        panel.style.setProperty("--inspira-panel-transform", `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) translateZ(0)`);
      };
      const onLeave = () => panel.style.removeProperty("--inspira-panel-transform");
      panel.addEventListener("pointermove", onMove);
      panel.addEventListener("pointerleave", onLeave);
      return { panel, onMove, onLeave };
    });
    return () => handlers.forEach(({ panel, onMove, onLeave }) => {
      panel.removeEventListener("pointermove", onMove);
      panel.removeEventListener("pointerleave", onLeave);
      panel.classList.remove("inspira-panel-surface");
      panel.style.removeProperty("--inspira-panel-transform");
    });
  });
}

function useInspiraButtonVariants() {
  useEffect(() => {
    const variantNames = ["inspira-gradient-button", "inspira-shimmer-button", "inspira-interactive-button", "inspira-ripple-button", "inspira-rainbow-button"];
    const page = document.querySelector(".main-content .page");
    if (!page) return undefined;
    const modelLearningPanels = document.querySelectorAll(".main-content .agent-learning-panel");
    const isModelWorkspace = modelLearningPanels.length > 0 && page.querySelector(".model-grid");
    const pageVariants = isModelWorkspace
      ? [".primary-button, .secondary-button", "inspira-ripple-button"]
      : page.classList.contains("page-dashboard")
      ? [".primary-button, .inspira-section-action", "inspira-gradient-button"]
      : page.classList.contains("forge-page") || page.classList.contains("team-lab-page")
        ? [".primary-button, .secondary-button, .candidate-load-more", "inspira-shimmer-button"]
        : page.classList.contains("arena-command-page")
          ? [".arena-primary-cta, .secondary-button, .team-source-switch button, .arena-segmented button", "inspira-rainbow-button"]
          : page.querySelector(".replay-layout")
            ? [".replay-viewer-tools .secondary-button", "inspira-interactive-button"]
            : page.querySelector(".rules-grid")
              ? [".page-title-row .secondary-button", "inspira-interactive-button"]
              : page.querySelector(".training-panel")
                ? [".primary-button, .secondary-button", "inspira-ripple-button"]
                : null;
    document.querySelectorAll(".inspira-gradient-button, .inspira-shimmer-button, .inspira-interactive-button, .inspira-ripple-button, .inspira-rainbow-button").forEach((element) => {
      variantNames.forEach((name) => element.classList.remove(name));
    });
    const variantRoot = isModelWorkspace ? document.querySelector(".main-content") : page;
    const controls = pageVariants ? Array.from(variantRoot.querySelectorAll(pageVariants[0])).map((element) => {
      const className = pageVariants[1];
      element.classList.add(className);
      if (className === "inspira-interactive-button") {
        element.dataset.inspiraLabel = element.textContent?.replace(/\s+/g, " ").trim() || "打开";
      }
      return { element, className };
    }) : [];
    const rippleControls = controls.filter(({ className }) => className === "inspira-ripple-button");
    const rippleHandlers = rippleControls.map(({ element }) => {
      const onClick = (event) => {
        const rect = element.getBoundingClientRect();
        const size = Math.max(rect.width, rect.height);
        const ripple = document.createElement("span");
        ripple.className = "inspira-ripple";
    ...
    return () => {
      controls.forEach(({ element, className }) => element.classList.remove(className));
      rippleHandlers.forEach(({ element, onClick }) => element.removeEventListener("click", onClick));
    };
  });
}

function TextGenerateEffect({ words, duration = 0.7, delay = 0, filter = true, className = "" }) {
  const [visible, setVisible] = useState(0);
  const parts = String(words || "").split(" ");
  useEffect(() => {
    setVisible(0);
    const timers = parts.map((_, index) => window.setTimeout(() => setVisible(index + 1), delay + index * 180));
    return () => timers.forEach((timer) => window.clearTimeout(timer));
  }, [words, delay]);
  return <span className={`inspira-text-generate ${className}`}>{parts.map((word, index) => <span key={`${word}-${index}`} className={index < visible ? "is-visible" : ""} style={{ transitionDuration: `${duration}s`, filter: filter ? undefined : "none" }}>{word}{index < parts.length - 1 ? " " : ""}</span>)}</span>;
}

const DEFAULT_SPARKLE_COLORS = ["#0ea5e9", "#f59e0b"];

function SparklesText({ text, sparklesCount = 10, colors = DEFAULT_SPARKLE_COLORS, className = "" }) {
  const [sparkles, setSparkles] = useState([]);
  useEffect(() => {
    const createSparkle = () => ({ id: `${Date.now()}-${Math.random()}`, left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, color: colors[Math.random() > 0.5 ? 0 : 1], delay: `${Math.random() * 1.8}s`, scale: 0.55 + Math.random() * 0.8 });
    const seed = () => setSparkles(Array.from({ length: sparklesCount }, createSparkle));
    seed();
    const timer = window.setInterval(seed, 2200);
    return () => window.clearInterval(timer);
  }, [sparklesCount, colors[0], colors[1]]);
  return <span className={`inspira-sparkles-text ${className}`}><span className="inspira-sparkles-copy">{text}</span>{sparkles.map((sparkle) => <span key={sparkle.id} className="inspira-sparkle-star" style={{ left: sparkle.left, top: sparkle.top, color: sparkle.color, animationDelay: sparkle.delay, "--sparkle-scale": sparkle.scale }} />)}</span>;
}
...
function VortexBackground() {
  ...
  useEffect(() => {
    ...
    let vortices = [];
    const makeVortices = () => [
      { x: width * .07, y: height * .38, strength: 1.3, radius: 230 },
      { x: width * .93, y: height * .52, strength: -1, radius: 260 },
      { x: width * .48, y: height * .9, strength: .8, radius: 210 },
      { x: width * .78, y: height * .18, strength: -.6, radius: 175 },
    ];
    const spawn = () => {
      const vortex = vortices[Math.floor(Math.random() * Math.max(1, vortices.length))];
      const nearVortex = vortex && Math.random() < .35;
      const angle = Math.random() * Math.PI * 2;
      const side = Math.floor(Math.random() * 4);
      const x = nearVortex ? vortex.x + Math.cos(angle) * Math.random() * vortex.radius : side === 1 ? width + 5 : side === 3 ? -5 : Math.random() * width;
      const y = nearVortex ? vortex.y + Math.sin(angle) * Math.random() * vortex.radius : side === 0 ? -5 : side === 2 ? height + 5 : Math.random() * height;
      return { x, y, vx: (Math.random() - .5) * 1.6, vy: (Math.random() - .5) * 1.6, size: Math.random() * 2.8 + .4, color: colors[Math.floor(Math.random() * colors.length)], opacity: Math.random() * .75 + .2, trail: [], trailLength: Math.floor(Math.random() * 28 + 10), life: 0, maxLife: Math.random() * 420 + 140 };
    };
    let particles = [];
    const resize = () => {
      ...
      particles = Array.from({ length: width < 700 ? 150 : 300 }, spawn);
    };
    const draw = () => {
      time += .0035;
      context.globalAlpha = 1;
      context.fillStyle = "rgba(6, 8, 24, .15)";
      context.fillRect(0, 0, width, height);
      vortices[0] = { ...vortices[0], x: width * .07 + Math.sin(time * .38) * 65, y: height * .38 + Math.cos(time * .28) * 42 };
      vortices[1] = { ...vortices[1], x: width * .93 + Math.cos(time * .32) * 52, y: height * .52 + Math.sin(time * .42) * 72 };
      vortices[2] = { ...vortices[2], x: width * .48 + Math.sin(time * .18) * 110, y: height * .9 + Math.cos(time * .48) * 28 };
      vortices[3] = { ...vortices[3], x: width * .78 + Math.cos(time * .26) * 58, y: height * .18 + Math.sin(time * .36) * 48 };
      vortices.forEach((vortex) => {
        const glow = context.createRadialGradient(vortex.x, vortex.y, 0, vortex.x, vortex.y, vortex.radius * .7);
        glow.addColorStop(0, "rgba(99,102,241,.09)"); glow.addColorStop(.5, "rgba(124,58,237,.05)"); glow.addColorStop(1, "transparent");
        context.beginPath(); context.arc(vortex.x, vortex.y, vortex.radius * .7, 0, Math.PI * 2); context.fillStyle = glow; context.fill();
      });
      particles.forEach((particle, index) => {
        vortices.forEach((vortex) => {
          const dx = vortex.x - particle.x; const dy = vortex.y - particle.y; const distance = Math.hypot(dx, dy);
          if (distance < vortex.radius && distance > 4) {
            const force = (vortex.radius - distance) / vortex.radius;
            particle.vx += (-dy / distance) * vortex.strength * force * .055 + (dx / distance) * force * .012;
            particle.vy += (dx / distance) * vortex.strength * force * .055 + (dy / distance) * force * .012;
          }
        });
        particle.vx += Math.sin(particle.x * .011 + time) * Math.cos(particle.y * .009 + time * .65) * .016;
        particle.vy += Math.cos(particle.x * .009 + time * .72) * Math.sin(particle.y * .011 + time * .38) * .016;
        particle.vx *= .972; particle.vy *= .972;
        const speed = Math.hypot(particle.vx, particle.vy);
        if (speed > 2.8) { particle.vx = particle.vx / speed * 2.8; particle.vy = particle.vy / speed * 2.8; }
        ...
        if (particle.x < -70 || particle.x > width + 70 || particle.y < -70 || particle.y > height + 70 || particle.life > particle.maxLife) { particles[index] = spawn(); return; }
        const progress = particle.life / particle.maxLife;
        const fade = progress < .12 ? progress / .12 : progress > .82 ? 1 - (progress - .82) / .18 : 1;
        if (particle.trail.length > 2) {
          context.beginPath(); context.moveTo(particle.trail[0].x, particle.trail[0].y); particle.trail.forEach((point) => context.lineTo(point.x, point.y));
          context.strokeStyle = particle.color; context.lineWidth = particle.size * .55; context.globalAlpha = particle.opacity * fade * .32; context.stroke();
        }
        context.beginPath(); context.arc(particle.x, particle.y, particle.size, 0, Math.PI * 2); context.fillStyle = particle.color; context.globalAlpha = particle.opacity * fade * .88; context.fill();
        if (particle.size > 1.4) {
          const halo = context.createRadialGradient(particle.x, particle.y, 0, particle.x, particle.y, particle.size * 5.5);
          halo.addColorStop(0, `${particle.color}55`); halo.addColorStop(1, `${particle.color}00`);
          context.beginPath(); context.arc(particle.x, particle.y, particle.size * 5.5, 0, Math.PI * 2); context.fillStyle = halo; context.globalAlpha = fade * .22; context.fill();
        }
        context.globalAlpha = 1;
      });
      frame = requestAnimationFrame(draw);
    };
    context.fillStyle = "#060818";
    resize(); context.fillRect(0, 0, width, height); draw(); window.addEventListener("resize", resize);
    return () => { cancelAnimationFrame(frame); window.removeEventListener("resize", resize); };
  }, []);
  return <div className="inspira-vortex" aria-hidden="true"><canvas ref={canvasRef} /></div>;
}
...
function SectionHeader({ eyebrow, title, action }) {
  const actionView = React.isValidElement(action) && action.props.className === "ghost-button"
    ? <GradientButton {...action.props} className={`inspira-section-action ${action.props.className || ""}`} />
    : action;
  return <div className="section-header"><div><span className="eyebrow">{eyebrow}</span><h2>{title}</h2></div>{actionView}</div>;
}

function TeamPreview({ team, onSelect }) {
  return <div className="team-preview-grid">{team.map((member) => <Card3D key={member.id} containerClassName="team-preview-3d" className="team-preview-3d-inner"><button className="preview-member" onClick={() => onSelect?.(member)}><Card3DItem translateZ={20}><div className={`preview-sprite tone-${member.tone}`}><Sprite id={member.sprite} /></div></Card3DItem><Card3DItem translateZ={34}><span>{member.name}</span></Card3DItem><Card3DItem translateZ={25}><small>{member.role}</small></Card3DItem></button></Card3D>)}</div>;
}

function LegacyDashboard({ team, onNavigate, agentState, onToggleAgent, onOpenAccount, registry }) {
...
function DashboardStat({ icon: Icon, tone, value, label, delta }) {
  return <article className="dashboard-stat"><div className={`dashboard-stat-icon is-${tone}`}><Icon size={17} /></div>{delta && <span className="dashboard-stat-delta">+{delta}</span>}<strong>{value}</strong><small>{label}</small></article>;
}

function CoverageBar({ label, value }) {
  return <div className="coverage-bar"><span>{label}</span><i><b style={{ width: `${value}%` }} /></i></div>;
}

function InfoRows({ title, rows = [] }) {
  return <><div className="dashboard-card-head"><h3>{title}</h3></div><div className="dashboard-info-rows">{rows.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></>;
}

function DashboardReplayRow({ item, index }) {
  const wins = Number(item.wins || 0);
  const losses = Number(item.losses || 0);
  const won = wins >= losses;
  const opponent = item.opponent || item.opponentName || item.username || "未知对手";
  const games = wins + losses + Number(item.ties || 0);
  const ratingChange = item.ratingChange ?? item.rating_delta ?? "--";
  const timestamp = item.finishedAt || item.createdAt;
  return <tr><td className="mono">{item.id || `M${String(index + 1).padStart(3, "0")}`}</td><td className="table-opponent">{opponent}</td><td><span className={`table-result ${won ? "is-win" : "is-loss"}`}>{won ? "胜利" : "失败"}</span></td><td className={won ? "positive mono" : "negative mono"}>{typeof ratingChange === "number" && ratingChange > 0 ? "+" : ""}{ratingChange}</td><td>{games || "--"} 回合</td><td className="mono">{timestamp ? new Date(timestamp).toLocaleString("zh-CN") : "--"}</td></tr>;
}

function Dashboard({ team, onNavigate, agentState, onToggleAgent, registry }) {
  const [overview, setOverview] = useState({ agent: null, replays: [], models: [] });
  const [error, setError] = useState("");
  useEffect(() => {
    let mounted = true;
    const refresh = async () => {
      try {
        const [agent, replayData, modelData] = await Promise.all([apiRequest("/api/agent/status"), apiRequest("/api/agent/replays"), apiRequest("/api/agent/models")]);
        if (mounted) setOverview({ agent, replays: replayData.items || [], models: modelData.items || [] });
  ...
  const activeRuleset = registry.active?.find((item) => item.battleType === "double") || registry.active?.[0];
  const replays = activeRuleset ? overview.replays.filter((item) => item.rulesetId === activeRuleset.rulesetId) : overview.replays;
  const totalGames = replays.reduce((sum, item) => sum + Number(item.games || 0) + Number(item.wins || 0) + Number(item.losses || 0), 0);
  const modelRegistry = overview.models.find((item) => item.rulesetId === activeRuleset?.rulesetId);
  const champion = modelRegistry?.champion?.version || overview.agent?.policyVersion || "尚未建立";
  const running = connectedAgentStatuses.has(overview.agent?.status) || agentState === "active";
  const moveIds = team.flatMap((member) => member.moves || []).map((move) => move.toLowerCase().replace(/[^a-z0-9]/g, ""));
  const countMoves = (names) => moveIds.filter((move) => names.includes(move)).length;
  const rating = overview.agent?.rating || overview.agent?.ladderRating || overview.agent?.elo || "--";
  const winCount = replays.reduce((sum, item) => sum + Number(item.wins || 0), 0);
  const winRate = totalGames ? `${Math.round((winCount / totalGames) * 1000) / 10}%` : "--";
  return <div className="page page-dashboard motion-fade-in">
    <div className="dashboard-heading"><div><h1><SparklesText text="Champion Forge" sparklesCount={5} colors={["#a78bfa", "#22d3ee"]} /></h1><p>竞技宝可梦战术工作台 · 总览面板</p></div><div className="dashboard-heading-actions"><StatusPill tone={running ? "green" : "muted"} icon={Activity}>{running ? "Agent 运行中" : "Agent 已暂停"}</StatusPill><button className="primary-button" onClick={() => onToggleAgent({ games: 1 })}><RefreshCw size={15} />刷新数据</button></div></div>
    <div className="dashboard-stat-grid"><DashboardStat icon={Gauge} tone="blue" value={rating} label="当前天梯分" delta={overview.agent?.ratingChange} /><DashboardStat icon={Activity} tone="green" value={winRate} label="总体胜率" delta={totalGames || undefined} /><DashboardStat icon={ShieldCheck} tone="violet" value={totalGames} label="总对局数" delta={replays.length || undefined} /><DashboardStat icon={Zap} tone="yellow" value={overview.agent?.winStreak || "--"} label="连胜场次" /></div>
    <div className="dashboard-main-grid">
      <section className="panel dashboard-team-card"><div className="dashboard-card-head"><h2>当前排位队伍 <span>{activeRuleset?.regulation || "当前规则"}</span></h2><button className="primary-button" onClick={() => onNavigate("forge")}>切换队伍</button></div><TeamPreview team={team} onSelect={() => onNavigate("forge")} /><div className="team-coverage-bars"><CoverageBar label="输出" value={Math.min(100, countMoves(["closecombat", "earthquake", "electroshot"]) * 22 + 40)} /><CoverageBar label="耐久" value={Math.min(100, countMoves(["protect", "wideguard", "detect"]) * 18 + 35)} /><CoverageBar label="速度" value={Math.min(100, countMoves(["tailwind", "icywind", "trickroom"]) * 22 + 38)} /><CoverageBar label="控场" value={Math.min(100, countMoves(["fakeout", "partingshot", "uturn"]) * 22 + 35)} /></div></section>
      <aside className="dashboard-side-stack"><section className="panel dashboard-info-card"><InfoRows title="Agent 状态" rows={[["在线状态", running ? "在线" : "已暂停"], ["当前操作", overview.agent?.lastAction || "等待对手"], ["使用模型", champion], ["运行时长", overview.agent?.uptime || "--"]]} /></section><section className="panel dashboard-info-card"><div className="dashboard-card-head"><h3>规则快照</h3><StatusPill tone={registry.canOperate ? "green" : "yellow"}>{registry.canOperate ? "正常" : "需同步"}</StatusPill></div><div className="dashboard-info-rows">{[["规则集", activeRuleset?.name || "--"], ["Regulation", activeRuleset?.regulation || "--"], ["格式 ID", activeRuleset?.showdownFormatId || "--"], ["规则哈希", activeRuleset?.formatHash ? `#${String(activeRuleset.formatHash).slice(0, 8)}` : "--"]].map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></section><section className="panel dashboard-info-card"><div className="dashboard-card-head"><h3>数据采集</h3><Database size={15} className="positive" /></div><div className="collection-row"><span>回放采集中</span><strong>{totalGames} 局</strong></div><div className="collection-progress"><span style={{ width: `${Math.min(100, (totalGames / 200) * 100)}%` }} /></div><small>目标 200 局 · {Math.min(100, Math.round((totalGames / 200) * 100))}% 完成</small></section></aside>
    </div>
    <section className="panel dashboard-recent-card"><div className="dashboard-card-head"><h2>最近对局</h2><Activity size={15} className="positive" /></div><div className="dashboard-table-wrap"><table className="dashboard-table"><thead><tr><th>对局编号</th><th>对手</th><th>结果</th><th>评分变化</th><th>回合数</th><th>时间</th></tr></thead><tbody>{replays.length ? replays.slice(0, 8).map((item, index) => <DashboardReplayRow key={`${item.finishedAt || item.createdAt || index}-${index}`} item={item} index={index} />) : <tr><td colSpan="6" className="empty-state compact">{error || "暂无真实对局记录"}</td></tr>}</tbody></table></div></section>
  </div>;
}
...
function BattleBoard({ team = [], status = {}, selectedFormat }) {
  ...
  return <>
    {!open && <motion.button type="button" className={`battle-ball-trigger board-state-${boardState.toLowerCase()}`} onClick={() => setOpen(true)} aria-label="打开对战面板" title="打开对战面板" initial={{ opacity: 0, scale: 0.78 }} animate={{ opacity: 1, scale: 1 }} whileHover={{ scale: 1.07, y: -3 }} whileTap={{ scale: 0.94 }} transition={{ type: "spring", stiffness: 420, damping: 24 }}><span /><i /><b>VS</b></motion.button>}
    {createPortal(<AnimatePresence>{open && <motion.section style={panelStyle} className={`battle-board board-state-${boardState.toLowerCase()} ${dragging ? "is-dragging" : ""}`} aria-label="交互式对战面板" initial={{ opacity: 0, scale: 0.96, y: 18 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 12 }} transition={{ type: "spring", stiffness: 360, damping: 30 }}>
    ...
    </motion.section>}</AnimatePresence>, document.body)}
  </>;
}
...
function ArenaCommandCenter({ team, agentState, onToggleAgent, onStop, registry }) {
  ...
  return <div className={`page arena-page arena-command-page arena-health-${String(health).toLowerCase()} motion-fade-in`}>
    ...
    <header className="arena-command-hero"><div className="arena-hero-copy"><div className="arena-kicker"><span className="arena-live-dot" />AGENT OPERATIONS <span>/</span> SHOWDOWN LADDER</div><h1><SparklesText text="竞技场控制台" sparklesCount={14} /></h1><p>从队伍来源到实时事件，都在同一个可追踪的操作面内完成。</p></div><div className="arena-hero-status"><div className="arena-status-orbit"><span /><span /><span /></div><div><span>当前状态</span><strong>{healthLabel}</strong><small>{status.lastServerEvent || "等待连接事件"}</small></div></div></header>
...
function TeamLab({ team, setTeam }) {
  ...
  return <div className="page team-lab-page">
    ...
    <section className="panel team-lab-panel"><SectionHeader eyebrow="SEARCHED CANDIDATES" title="候选队伍" action={<span className="mono muted">STRICT SEARCH + LOCAL SHOWDOWN</span>} />{data?.experiments?.length ? <div className="team-lab-grid">{data.experiments.map((candidate, index) => { const evaluation = candidate.evaluation || {}; const canPromote = evaluation.ok && Number(evaluation.games || 0) >= 4 && candidate.status !== "promoted"; return <motion.article className="team-lab-card" key={candidate.id} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", stiffness: 330, damping: 28, delay: Math.min(index, 7) * 0.045 }} whileHover={{ y: -4 }}><div className="team-lab-card-head"><div><span className="eyebrow">{candidate.status === "promoted" ? "PROMOTED" : "CANDIDATE"}</span><h2>{candidate.id}</h2></div><StatusPill tone={candidate.validation?.ok ? "green" : "red"}>{candidate.validation?.ok ? "规则合法" : "待修复"}</StatusPill></div><div className="team-lab-members">{(candidate.team || []).map((member) => <div key={`${candidate.id}-${member.slug}-${member.item}`}><div className="team-lab-sprite"><Sprite id={member.slug || member.name} size="sm" /></div><strong>{member.name || member.slug}</strong><small>{member.role || "结构成员"}</small></div>)}</div><div className="team-lab-stats"><div><span>结构分</span><b>{Number(candidate.score || 0).toFixed(1)}</b></div><div><span>本地评估</span><b>{evaluation.games ? `${evaluation.winRate}%` : "未评估"}</b></div><div><span>战绩</span><b>{evaluation.games ? `${evaluation.wins}-${evaluation.losses}` : "-"}</b></div></div><div className="team-lab-note">{candidate.buildReport?.plan || "已通过当前规则的严格配置搜索。"}</div><div className="team-lab-actions">{canPromote && <button className="primary-button" onClick={() => promote(candidate.id)} disabled={Boolean(busy)}><Trophy size={15} />{busy === candidate.id ? "晋级中..." : "晋级为 Champion"}</button>}{!evaluation.ok && <span className="muted">需要更多本地靶队评估后才能晋级</span>}{candidate.status === "promoted" && <span className="positive"><Check size={15} />已进入规则队伍注册表</span>}</div></motion.article>; })}</div> : <div className="empty-state">还没有候选队伍。点击“生成候选队伍”开始当前规则下的搜索实验。</div>}</section>
...
function App() {
  ...
  const withBackground = (view) => view;
  const content = useMemo(() => ({
    dashboard: withBackground(<Dashboard team={team} onNavigate={setPage} agentState={agentState} onToggleAgent={toggleAgent} onOpenAccount={() => setAccountOpen(true)} registry={registry} />),
    forge: withBackground(<Forge team={team} setTeam={setTeam} onNavigate={setPage} />),
    teamlab: withBackground(<TeamLab team={team} setTeam={setTeam} />),
    arena: withBackground(<ArenaCommandCenter team={team} agentState={agentState} onToggleAgent={toggleAgent} onStop={stopAgent} registry={registry} />),
    replays: withBackground(<Replays />),
    rules: withBackground(<Rules />),
    models: withBackground(<><Models />{(registry.active || []).filter((snapshot) => snapshot.status === "active" && (snapshot.battleType === "single" || snapshot.battleType === "double")).map((snapshot) => <AgentLearningPanel key={snapshot.rulesetId} format={snapshot.battleType} rulesetId={snapshot.rulesetId} />)}</>),
  }[page]), [agentState, page, registry, team]);
  return <div className={`app-shell ${agentState === "paused" ? "agent-paused" : ""}`}><VortexBackground /><div className="app-bg-layer" aria-hidden="true" /><div className="sr-only" aria-live="assertive">{announcement}</div><header className="topbar"><div className="brand"><div className="brand-mark"><span /></div><strong>Champion Forge</strong><span className="desktop-only brand-sub">Competitive Agent Workbench</span></div><div className="top-status"><StatusPill tone={registry.status === "ACTIVE" ? "blue" : "yellow"} icon={BookOpen}>{activeRuleset?.name?.replace(/^\[Gen \d+ Champions\]\s*/, "") || registry.status}</StatusPill><StatusPill tone={registry.canOperate ? "green" : "yellow"} icon={Activity}>{registry.canOperate ? "RULES SYNCED" : "RULES BLOCKED"}</StatusPill><StatusPill tone={agentState === "active" ? "green" : "muted"} icon={Bot}>{agentState === "active" ? <span className="agent-breath">Agent active</span> : agentState === "starting" ? "Agent starting" : "Agent paused"}</StatusPill></div><div className="top-actions"><button className="top-account" onClick={() => setAccountOpen(true)} aria-label="账号设置"><span className="account-avatar"><Bot size={15} /></span><span className="desktop-only">专用账号</span></button><button className={`kill-switch ${isKilled ? "kill-flash" : ""}`} onClick={stopAgent} aria-label="紧急停止 Agent"><CircleStop size={15} /> <span className="desktop-only">KILL SWITCH</span><kbd>Ctrl ⇧ K</kbd></button><button className="mobile-menu icon-button" aria-label="打开菜单"><Menu size={19} /></button></div></header>{announcement && <div className={`app-notice notice-${announcementTone}`} role="status">{announcementTone === "error" ? <AlertTriangle size={16} /> : announcementTone === "success" ? <Check size={16} /> : <Bot size={16} />}<span>{announcement}</span><button className="icon-button" onClick={() => setAnnouncement("")} aria-label="关闭状态提示"><X size={15} /></button></div>}<div className="shell-body"><aside className="sidebar" aria-label="主导航"><div className="sidebar-brand"><div className="sidebar-brand-mark"><Zap size={16} /></div><div><strong>CHAMPION FORGE</strong><small>竞技战术工作台</small></div></div><div className="nav-group">{navItems.map(([id, label, Icon]) => <button key={id} className={`nav-item ${page === id ? "is-active" : ""}`} onClick={() => setPage(id)} aria-current={page === id ? "page" : undefined}><Icon size={18} /><span>{label}</span>{page === id && <i />}</button>)}</div><div className="sidebar-foot"><button className="nav-item" onClick={() => setAccountOpen(true)}><Settings size={18} /><span>账号与设置</span></button><div className="sidebar-agent-state"><span className={`dot ${agentState === "active" ? "dot-green" : "dot-yellow"}`} /><div><strong>{agentState === "active" ? "Agent 在线" : "Agent 已暂停"}</strong><small>{registry.canOperate ? "Showdown 已连接" : "规则等待同步"}</small></div></div><div className="sync-card"><div><span className={`dot ${registry.canOperate ? "dot-green" : "dot-yellow"}`} />当前赛季</div><strong>{activeRuleset?.name?.replace(/^\[Gen \d+ Champions\]\s*/, "") || "等待规则"}</strong><small>{activeRuleset?.regulation || "等待同步"}</small></div></div></aside><main className="main-content">{content}</main></div>{accountOpen && <AccountWizard onClose={() => setAccountOpen(false)} />}</div>;
}
